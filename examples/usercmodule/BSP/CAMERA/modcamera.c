...
static bool camera_init_helper(camera_obj_t *camera, size_t n_pos_args, const mp_obj_t *pos_args, mp_map_t *kw_args)
{
    ...
    static const mp_arg_t allowed_args[] = {
        ...
        {MP_QSTR_quality, MP_ARG_KW_ONLY | MP_ARG_INT, {.u_int = 5}},
    ...
    camera->config.fb_count = 2;      // if more than one, i2s runs in continuous mode. Use only with JPEG
...
static uint8_t *jpeg = NULL;
static mp_obj_t camera_capture_rgb565()
{
    // acquire a frame
    camera_fb_t *fb = esp_camera_fb_get();
    if (!fb)
    ...
    frame2jpg(fb, 40, &jpeg, &jpeg_len);
    mp_obj_t image = mp_obj_new_bytes(jpeg, jpeg_len);
    if (jpeg)
    {
        free(jpeg);
        jpeg = NULL;
...
static MP_DEFINE_CONST_FUN_OBJ_0(camera_capture_rgb565_obj, camera_capture_rgb565);
...
static const mp_rom_map_elem_t camera_module_globals_table[] = {
    {MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_esp_camera)},

    {MP_ROM_QSTR(MP_QSTR_init), MP_ROM_PTR(&camera_init_obj)},
    {MP_ROM_QSTR(MP_QSTR_deinit), MP_ROM_PTR(&camera_deinit_obj)},
    {MP_ROM_QSTR(MP_QSTR_capture), MP_ROM_PTR(&camera_capture_obj)},
    {MP_ROM_QSTR(MP_QSTR_capture_rgb565), MP_ROM_PTR(&camera_capture_rgb565_obj)},
...
MP_REGISTER_MODULE(MP_QSTR_esp_camera, mp_module_camera_system);
